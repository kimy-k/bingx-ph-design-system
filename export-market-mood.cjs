...
const BASE = path.join(__dirname, 'library', 'market-mood');
const OUT = path.join(__dirname, 'library', 'exports', 'market-mood');
...
async function main() {
  ...
  console.log('\n🌤️  Market Mood PNG Export (full bleed)');
  console.log(`   ${cards.length} templates × ${Object.keys(SIZES).length} formats = ${cards.length * Object.keys(SIZES).length} PNGs\n`);
  ...
  for (const card of cards) {
    ...
    for (const [htmlFile, dims] of Object.entries(SIZES)) {
      ...
      try {
        ...
        await page.close();
        exported++;
        if (exported % 6 === 0) console.log(`   ✅ ${exported} / ${cards.length * 3} exported...`);
      } catch (err) {
