...
const BASE = path.join(__dirname, 'library', 'safety-cards');
const OUT = path.join(__dirname, 'library', 'exports', 'safety-cards');
...
async function main() {
  ...
  console.log('\n🛡️  Safety Card PNG Export (full bleed)');
    ...
    for (const [htmlFile, dims] of Object.entries(SIZES)) {
      ...
      try {
        ...
        await page.screenshot({
          path: outFile, type: 'png',
          clip: { x: 0, y: 0, width: dims.width, height: dims.height }
        });

        await page.close();
        exported++;
        if (exported % 10 === 0) console.log(`   ✅ ${exported} / ${cards.length * 4} exported...`);
      } catch (err) {
